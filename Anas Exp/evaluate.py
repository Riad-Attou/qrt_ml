...
def main():
    ...
    y_test = target_train.set_index("ID").loc[X_test.index]

    # Charger le modèle
    cox_model = train_cox_model(X_test, y_test)

    # Évaluer le modèle
    c_index = evaluate_model(cox_model, X_test, y_test)
    print(f"C-index du modèle : {c_index}")
